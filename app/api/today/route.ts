...
export function GET() {
  const date = todayETDateString();
  const rng = seededRng(date);

  // Try several Ns until we find one covered by >= 5 categories
  ...
  // Fallback: choose rank with max coverage
...
const clues: Clue[] = cats.map(cat => {
  ...
  return {
    category_id: cat.id,
    category_name: cat.name,   // ⬅️ added
    source: cat.source,        // ⬅️ added
    item,
    hint_item: hintItem
  };
});
  ...
  return NextResponse.json(puzzle, { headers: { "Cache-Control": "s-maxage=60" }});
}
